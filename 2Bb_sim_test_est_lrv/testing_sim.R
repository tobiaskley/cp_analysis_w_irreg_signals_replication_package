...
for(i in 1:nexp){
  ...
  rej_alt_asy <- c(rej_alt_asy, test_CP(data, asymptotic = TRUE,
                                        quantile_val = quantile_asy, J = 3)$REJECT)
  rej_alt_fin <- c(rej_alt_fin, test_CP(data, asymptotic = TRUE,
...
result_alt <- c(n = n,
                ...
                theta = theta,
                sigma = z_prime_sigma,
                power_asy = mean(rej_alt_asy),
                power_fin = mean(rej_alt_fin)
)
